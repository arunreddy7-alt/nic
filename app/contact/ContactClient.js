  ...
  const handleSubmit = async (e) => {
    ...
    emailjs.send(
      ...
      'template_2fe0lt9',
      {
        ...
        project_name: newBooking.projectName,
        location: newBooking.location,
        city: newBooking.city,
        requirements: newBooking.requirements,
        services: newBooking.services.join(', '),
      ...
      <section className="min-h-screen bg-[#845547] text-[#fffbea] flex items-center justify-center py-34 px-6">
        ...
        <div>
          ...
          <form onSubmit={handleMainContactSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-xs font- mb-1">
                Phone Number <span className="text-xs opacity-70">(required)</span>
              </label>
              <input
                type="tel"
                name="phone"
                value={mainContactFormData.phone}
                onChange={handleMainContactInputChange}
                required
                className="w-full border border-[#fffbea]/30 bg-white p-2 mt-1 focus:outline-none focus:ring-1 focus:ring-[#fffbea] text-black"
              />
        ...
        <div className="fixed inset-0 bg-opacity-0 backdrop-blur-sm flex items-center justify-center z-50 pointer-events-auto">
          <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Attach Plan (PDF/Image)</label>
                <input
                  type="file"
                  name="attachPlan"
                  onChange={handleInputChange}
                  accept=".pdf,.jpg,.jpeg,.png"
                  className="w-full border border-gray-300 p-2 rounded bg-gray-50 text-gray-600 cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-[#845547] file:text-white hover:file:bg-[#6a3f3a]"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {['Proper scouting', 'Architecture', 'Interiors', 'Project management'].map(service => (
                    <label key={service} className="flex items-center text-black">
                      <input
                        type="checkbox"
                        name="services"
                        value={service}
